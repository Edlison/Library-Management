...
from backend.api.user.mapper.user_mapper import get_user_by_name, add_user_borrowing, add_user_reser
from backend.api.circulation.mapper.circulation_mapper import get_book_by_isbn, add_borrow, add_reser, reduce_remainder
    ...
    if user:
        if user.user_reserving is not None and user.user_reserving < 1:
            if book.book_remainder_num is not None and book.book_remainder_num > 1:
                ...
                res = SystemResult().ok('预约成功')
            else:
                res = SystemResult().error('图书已无余量')
        else:
            res = SystemResult().error('预约已达上限 不能预约')
    else:
        res = SystemResult().error('用户错误')
    return res