...
def search_book_ISBN(book_ISBN):
    catalog_list = Catalog.query.filter_by(Catalog.book_ISBN == book_ISBN)
    return catalog_list

def search_catalog_id(catalog_id):
    catalog_list = Catalog.query.filter_by(Catalog.catalog_id == catalog_id)
    return catalog_list
...
def search_state():
    # 获得采访清单内所有的内容
    catalog_list = Catalog.query.filter_by(Catalog.book_state == 0)
    # 用来存储转换成json格式的列表
    catalog_jsonlist = []
    # 转换格式并存储
    for each in catalog_list:
...
def update_book_num(book_state,book_num):
    catalog = Catalog.query.filter_by(Catalog.book_state == book_state)
